...
conf = SparkConf().setAppName('valoracionesTripAdvisorBcn')
sc = SparkContext(conf = conf)
spark = SparkSession(sc)

path = "trip_advisor_barcelona.csv"
...
dfMejorValorados.toPandas().to_excel("restaurantes_mejor_valorados_barcelona.xlsx", index=False)
dfPeorValorados.toPandas().to_excel("restaurantes_peor_valorados_barcelona.xlsx", index=False)
...
plt.title("Valoraciones Barcelona")
plt.show()
